...
/** Editor. Export sengaja dipisahkan ke ProcessActivity supaya progress tidak
 * menjadi popup kecil dan proses tetap jelas ketika layar berubah. */
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        b.swEnhance.setOnCheckedChangeListener { _, on ->
            opts = opts.copy(enhance = on); updatePlanText()
        }
        b.swStab.setOnCheckedChangeListener { _, on ->
            opts = opts.copy(stabilize = on); updatePlanText()
            toast(if (on) "Stabilisasi ringan aktif — video akan sedikit di-crop"
    ...
    private fun pickRes() {
        ...
        val labels = items.map {
            when (it) {
                ...
                Converter.Res.SOURCE -> if (srcH > 0) "Asli · ${srcW}x${srcH} — file bisa besar" else "Asli"
            }
        }.toTypedArray()
    ...
    private fun loadVideo(uri: Uri) = lifecycleScope.launch {
        val duration = withContext(Dispatchers.IO) { Converter.videoDurationMs(this@MainActivity, uri) }
        if (duration <= 0) { toast("Video ini tidak bisa dibaca"); finish(); return@launch }
        val (w, h) = withContext(Dispatchers.IO) { Converter.videoSize(this@MainActivity, uri) }
        videoUri = uri; srcW = w; srcH = h; plan = Converter.plan(duration)
        updatePlanText(); refreshPreview()
    }

    private fun outDim(): Pair<Int, Int> {
        val h = evenUp(opts.heightFor(srcH.takeIf { it > 0 } ?: 1080).toFloat())
        if (opts.square) return h to h
        val ratio = if (srcW > 0 && srcH > 0) srcW.toFloat() / srcH else 9f / 16f
        return evenUp(h * ratio) to h
    }

    private fun evenUp(v: Float): Int {
        var out = v.toInt().coerceAtLeast(2)
        if (out % 2 != 0) out++
        return out
    }
    ...
    private fun updatePlanText() {
        ...
        b.tvPlan.text = "Sumber   : ${srcW}x${srcH}, %.1f dtk\n".format(p.totalMs / 1000f) +
            "Diambil  : %.1f – %.1f dtk\n".format(p.startMs / 1000f, (p.startMs + p.durationMs) / 1000f) +
    ...
    private fun refreshPreview() {
        ...
        previewJob = lifecycleScope.launch {
            val bmp = withContext(Dispatchers.IO) {
                Converter.extractFrame(this@MainActivity, uri, p.startMs + p.keyframeOffsetMs, opts, 640)
            }
    ...
    private fun openExport() {
        val uri = videoUri ?: return
        if (plan == null) { toast("Video masih disiapkan"); return }
