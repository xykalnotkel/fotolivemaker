...
import livefoto.xyspace.app.databinding.ActivityProcessBinding
...
/**
 * Layar export video HD/UHD terpisah ala CapCut
 */
class VideoProcessActivity : AppCompatActivity() {
    companion object {
    ...
    private fun loadPreview(uri: Uri, opts: Converter.Options, hint: Converter.Plan?) =
        lifecycleScope.launch {
            ...
            val bmp = withContext(Dispatchers.IO) {
                val (sourceW, sourceH) = Converter.videoSize(this@VideoProcessActivity, uri)
                ...
                Converter.extractFrame(
                    this@VideoProcessActivity, uri, at, opts, previewW, previewH
                )
            }
        ...
        job = lifecycleScope.launch {
            try {
                Converter.convertVideo(
                    this@VideoProcessActivity, uri, opts,
                    ...
                    b.tvStage.text = "Video selesai - ${result.width}x${result.height}"
                    b.tvEta.text = "Selesai"
                    b.btnCancel.visibility = View.GONE
                    startActivity(
                        Intent(this@VideoProcessActivity, VideoResultActivity::class.java)
                            .putExtra(VideoResultActivity.EXTRA_URI, result.uri.toString())
                            .putExtra(VideoResultActivity.EXTRA_WIDTH, result.width)
                            .putExtra(VideoResultActivity.EXTRA_HEIGHT, result.height)
                    )
                    finish()
                }
